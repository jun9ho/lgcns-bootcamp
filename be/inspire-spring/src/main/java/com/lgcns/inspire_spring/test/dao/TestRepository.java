...
public class TestRepository {
    ...
    public TestResponseDTO testRow(TestRequestDTO request) {
        ...
        String testSQL = "SELECT * " + "FROM employee" + "WHERE emp_id = "+request.getEmp_id()+"AND emp_name = "+request.getEmail();
        System.out.println("[debug] >>> " + testSQL);

        TestResponseDTO response = TestResponseDTO.builder().emp_id("100").emp_name("한선기").salary(900).build();
        return response;
    }
}
