...
SELECT STUDENT_NAME
FROM tb_student
WHERE DEPARTMENT_NO = 001 AND ABSENCE_YN ='Y' AND SUBSTRING(STUDENT_SSN,8,1) IN(2,4);
...
FROM tb_student
WHERE INSTR(STUDENT_ADDRESS,'전주')  AND SUBSTRING(STUDENT_NO,1,2)='A2' AND ABSENCE_YN = 'N';

SELECT STUDENT_NO,STUDENT_NAME,STUDENT_SSN
FROM tb_student
WHERE STUDENT_ADDRESS LIKE '전주%'  AND ENTRANCE_DATE LIKE '2002%' AND ABSENCE_YN = 'N';
-- sutdnet address에서 전주가 있어야하고, 학번은 A2가 있어야하며 ABSENCE_YN은 N
